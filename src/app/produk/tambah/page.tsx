...
export default function TambahProdukPage() {
  ...
  const fetchInitialData = async () => {
    ...
    try {
      // Fetch distributors
      const distDocRef = doc(database, "migrated_data", "Ao1AOituA9BLIVdduMKe");
      const distDocSnap = await getDoc(distDocRef);
      if (distDocSnap.exists() && distDocSnap.data()?.data) {
        setDistributors(distDocSnap.data().data);
      }

      // Fetch categories
      const catDocRef = doc(database, "migrated_data", "XgGV6xmYVMHK9SDuc1s0");
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
        const productDocRef = doc(database, "migrated_data", "4McBCfDf5XJnXnw2x8Xm");

        // Although not ideal for concurrency, this is how the existing structure works.
        // A better approach would be a dedicated `products` collection.
        await updateDoc(productDocRef, {
            data: arrayUnion({
              ...
              warning: formData.warning || "0",
              // Generate a somewhat unique ID if not provided
