  ...
  const handleFetchData = async () => {
    ...
    try {
      ...
      if (querySnapshot.empty) {
        setFirebaseData("No data available in the 'migrated_data' collection.");
      } else {
        const data = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        setFirebaseData(JSON.stringify(data, null, 2));
      }
    } catch (error: any) {
      const friendlyMessage = error.code === 'permission-denied'
          ...
          <div className="lg:col-span-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Check the current data in your Firestore.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="border rounded-md p-2 bg-muted/50 min-h-[200px] overflow-auto">
                  {isFetchingData ? (
                    ...
                    <pre className="text-xs">{firebaseData || "Click 'Check Database' to fetch data."}</pre>
                  )}
                </div>
              </CardContent>
              <CardFooter>
                <Button onClick={handleFetchData} disabled={isFetchingData} className="w-full">
                  {isFetchingData ? (
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                  ) : null}
                  Check Database
